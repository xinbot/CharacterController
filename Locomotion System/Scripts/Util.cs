...
public class Util
{
    ...
    {
        if (System.Single.IsNaN(f) || float.IsPositiveInfinity(f) || float.IsNegativeInfinity(f)) return false;
    ...
    public static float[] GetLineSphereIntersections(Vector3 lineStart, Vector3 lineDir, Vector3 sphereCenter,
        float sphereRadius)
    {
        float a = lineDir.sqrMagnitude;
        float b = 2 * (Vector3.Dot(lineStart, lineDir) - Vector3.Dot(lineDir, sphereCenter));
        float c = lineStart.sqrMagnitude + sphereCenter.sqrMagnitude - 2 * Vector3.Dot(lineStart, sphereCenter) -
