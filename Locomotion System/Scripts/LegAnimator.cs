...
public class LegAnimator : MonoBehaviour
{
    ...
    [Space] public float maxFootRotationAngle = 45.0f;
    ...
    [Header("Debug")] public bool renderFootMarkers = false;
    public bool renderBlendingGraph = false;
    public bool renderCycleGraph = false;
    public bool renderMotionCycle = false;
    public bool renderAnimationStates = false;
    public bool renderTrajectories = false;
    ...
    [System.NonSerialized] public GameObject ghost;

    private Dictionary<string, TrajectoryVisualizer> trajectories = new();
    ...
    private void Update()
    {
        ...
        objectVelocity = (transform.InverseTransformPoint(tr.velocitySmoothed) -
        ...
        for (int g = 0; g < legC.motionGroups.Length; g++)
        {
            ...
            if (newVelocity || changedGroupWeight)
            {
                ...
                if ((newVelocity || justEnabled) && group.weight > 0)
                {
                    ...
                    group.relativeWeights =
                        groupInfo.GetMotionWeights(new Vector3(objectVelocity.x, 0, objectVelocity.z));
                }
            }
            ...
            if (group.weight > 0)
            {
                ...
                {
                    if (blendSmoothing > 0)
                        group.relativeWeightsBlended[m] = Mathf.Lerp(group.relativeWeightsBlended[m],
                            group.relativeWeights[m], Time.deltaTime / blendSmoothing);
                    else
                        group.relativeWeightsBlended[m] = group.relativeWeights[m];
                    ...
                    AssertSane(Time.deltaTime / blendSmoothing,
                        "Time.deltaTime / blendSmoothing ( " + Time.deltaTime + " / " + blendSmoothing + " )");
    ...
    private void LateUpdate()
    {
        ...
        for (int leg = 0; leg < legs.Length; leg++)
        {
            ...
            if (System.Single.IsNaN(basePointFoot[leg].x) || System.Single.IsNaN(basePointFoot[leg].y) ||
                System.Single.IsNaN(basePointFoot[leg].z))
            {
                UnityEngine.Debug.LogError("legStates[leg].cycleTime=" + legStates[leg].cycleTime + ", strideSCurve=" +
                                           strideSCurve + ", tangentDir=" + tangentDir + ", cycleDistance=" +
                                           cycleDistance + ", legStates[leg].stepFromPosition=" +
                                           legStates[leg].stepFromPosition + ", legStates[leg].stepToPosition=" +
                                           legStates[leg].stepToPosition +
                                           ", legStates[leg].stepToMatrix.MultiplyVector(tangentDir)=" +
                                           legStates[leg].stepToMatrix.MultiplyVector(tangentDir) +
        ...
        for (int leg = 0; leg < legs.Length; leg++)
        {
            ...
            legStates[leg].footBase +=
                Mathf.Max(0, heightMidOffset * flightProgressionLift - flightPos.y * scale) * legsUp;
            AssertSane(legStates[leg].footBase, "legStates[leg].footBase");
    ...
    private Matrix4x4 FindGroundedBase(Vector3 pos, Quaternion rot, Vector3 heelToToetipVector, bool avoidLedges)
    {
        RaycastHit hit;
    ...
    private void AdjustLeg(int leg, Vector3 desiredAnklePosition, bool secondPass)
    {
    ...
    private void OnRenderObject()
    {
        ...
        if (isActive)
        {
            ...
            if (renderMotionCycle) RenderMotionCycles();
            if (renderTrajectories) RenderTrajectories();
        }
    }

    private void RenderBlendingGraph()
    {
    ...
    private void RenderCycleGraph()
    {
        float w = Camera.main.pixelWidth;
        float h = Camera.main.pixelHeight;
        //Color strongColor = new Color(0, 0, 0, 1);
        Color weakColor = new Color(0.7f, 0.7f, 0.7f, 1);
        DrawArea graph = new DrawArea(new Vector3(w - 0.49f * h, 0.01f * h, 0),
            new Vector3(w - 0.01f * h, 0.49f * h, 0));
    ...
    private void RenderMotionCycles()
    {
        //float w = Camera.main.pixelWidth;
        float h = Camera.main.pixelHeight;
        //Color strongColor = new Color(0, 0, 0, 1);
    ...
    private void RenderFootMarkers()
    {
        ...
        for (int leg = 0; leg < legs.Length; leg++)
        {
            for (int step = 0; step < 3; step++)
            {
                ...
                if (step == 2)
                {
                    heel += legStates[leg].stepToPositionGoal - legStates[leg].stepToPosition;
                }
    ...
    private void RenderAnimationStates()
    {
        int i = 0;
