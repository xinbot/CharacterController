...
class LocomotionEditorClass : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        lc.groundedPose =
            EditorGUILayout.ObjectField("Grounded Pose", lc.groundedPose, typeof(AnimationClip),
        ...
        if (GUILayout.Button("Initialize"))
        {
            ...
            if (success)
                lc.InitMotionAnalyzer();
        }

        EditorGUILayout.EndHorizontal();
    }

    private void LegSectionGUI()
    {
        // Handle legs array
    ...
    private void AnimationSectionGUI()
    {
        ...
        if (animsFoldout)
        {
            ...
            if (rebuildGroups || groups == null)
            {
                ...
                List<AnimationClip> unusedClips =
                    new List<AnimationClip>(AnimationUtility.GetAnimationClips(lc.gameObject));
            ...
            for (int g = 1; g < groups.Count; g++)
            {
                ...
                group.expanded =
                    GUILayout.Toggle(group.expanded, "", EditorStyles.foldout, GUILayout.ExpandWidth(false));
                if (g == 1)
                {
                ...
                if (group.expanded)
                {
                    ...
                    for (int m = 0; m < group.motions.Count; m++)
                    {
                        ...
                        if (used)
                        {
                            ...
                            expanded = GUILayout.Toggle(expandedOld, "", EditorStyles.foldout,
                            ...
                            GUI.changed = false;
                            ma.animation = EditorGUILayout.ObjectField(ma.animation, typeof(AnimationClip), false,
                                GUILayout.ExpandWidth(true)) as AnimationClip;
    ...
    private static bool SanityCheckAnimationCurves(LegController legC, AnimationClip animation)
    {
        ...
        if (!hasRootPosition)
        {
            Debug.LogError("AnimationClip \"" + animation.name +
                           "\" is missing animation curve for the position of the root bone \"" + legC.root.name +
                           "\".");
            success = false;
        }
        ...
        {
            Debug.LogError("AnimationClip \"" + animation.name +
                           "\" is missing animation curve for the rotation of the root bone \"" + legC.root.name +
                           "\".");
            success = false;
        }
        ...
        for (int i = 0; i < legC.legs.Length; i++)
        {
            for (int j = 0; j < legC.legs[i].legChain.Length; j++)
            {
                ...
                {
                    Debug.LogError("AnimationClip \"" + animation.name +
                                   "\" is missing animation curve for the rotation of the joint \"" +
                                   legC.legs[i].legChain[j].name + "\" in leg " + i + ".");
                    success = false;
                }
            }
        }
