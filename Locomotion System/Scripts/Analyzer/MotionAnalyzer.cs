...
public class MotionAnalyzer : IMotionAnalyzer
{
    ...
    {
        get { return m_cycleDistance; }
    }

    public override Vector3 cycleVector
    {
        get { return m_cycleDirection * m_cycleDistance; }
    }
    ...
    {
        get { return m_cycleDirection * m_cycleSpeed; }
    }
    ...
    private float FindContactTime(LegCycleData data, bool useToe, int searchDirection, float yRange, float threshold)
    {
        ...
        for (int i = 0; i < samples && i > -samples; i += searchDirection)
        {
            ...
            float curvatureCurrent = Mathf.Atan((value[2] - value[1]) * 10 / yRange) -
                                     Mathf.Atan((value[1] - value[0]) * 10 / yRange);
            if (
    ...
    public override void Analyze(GameObject o)
    {
        ...
        for (int leg = 0; leg < legs; leg++)
        {
            ...
            if (motionType == MotionType.WalkCycle)
            {
                ...
                Vector3 stanceSlideVector = (
                    cycles[leg].samples[GetIndexFromTime(Util.Mod(cycles[leg].liftoffTime + cycles[leg].stanceTime))]
                        .footBase
                    - cycles[leg].samples[GetIndexFromTime(Util.Mod(cycles[leg].strikeTime + cycles[leg].stanceTime))]
                        .footBase
                );
                // FIXME: Assumes horizontal ground plane
                stanceSlideVector.y = 0;
                cycles[leg].cycleDistance =
                    stanceSlideVector.magnitude / (cycles[leg].liftoffTime - cycles[leg].strikeTime + 1);
        ...
        for (int leg = 0; leg < legs; leg++)
        {
            ...
            Debug.Log("Cycle direction of leg " + leg + " is " + cycles[leg].cycleDirection + " with step distance " +
                      cycles[leg].cycleDistance);
        }
        ...
        Debug.Log("Overall cycle direction is " + m_cycleDirection + " with step distance " + m_cycleDistance +
                  " and speed " + m_cycleSpeed);
            ...
            if (motionType == MotionType.WalkCycle)
            {
                for (int j = 0; j < samples; j++)
                {
                    ...
                    if (fixFootSkating)
                    {
                        ...
                        if (time <= cycles[leg].liftoffTime)
                        {
                            s.footBaseNormalized.z = 0;
                        }

                        if (time >= cycles[leg].strikeTime)
                        {
                            s.footBaseNormalized.z = 1;
                        }
